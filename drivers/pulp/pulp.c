...
static int __init pulp_init(void) {
  ...
  my_dev.periph = ioremap_nocache(PERIPHERALS_H_BASE_ADDR, PERIPHERALS_SIZE_B);
  printk(KERN_INFO "PULP: Peripherals mapped to virtual kernel space @ %#lx.\n",(long unsigned int) my_dev.periph); 
  ...
  my_dev.config = ioremap_nocache(CONFIG_BASE_ADDR, CONFIG_SIZE_B);
  printk(KERN_INFO "PULP: Config mapped to virtual kernel space @ %#lx.\n",(long unsigned int) my_dev.config); 
 ...
 fail_request_irq:
  ...
  iounmap(my_dev.periph);
  iounmap(my_dev.l2_mem);
  iounmap(my_dev.config);
  iounmap(my_dev.slcr);
...
int pulp_mmap(struct file *filp, struct vm_area_struct *vma) {
  ...
  else if (off < (L3_MEM_SIZE_B + CLUSTERS_SIZE_B + PERIPHERALS_SIZE_B)) {
    // Peripherals
    base_addr = PERIPHERALS_H_BASE_ADDR;
    off = off - L3_MEM_SIZE_B - CLUSTERS_SIZE_B;
    size_b = PERIPHERALS_SIZE_B;
    strcpy(type,"Peripherals");
  }
  else if (off < (L3_MEM_SIZE_B + CLUSTERS_SIZE_B + PERIPHERALS_SIZE_B + L2_MEM_SIZE_B)) {
    // L2
    base_addr = L2_MEM_H_BASE_ADDR;
    off = off - L3_MEM_SIZE_B - CLUSTERS_SIZE_B - PERIPHERALS_SIZE_B;
    size_b = L2_MEM_SIZE_B;
    strcpy(type,"L2");
  }
  else if (off < (L3_MEM_SIZE_B + CLUSTERS_SIZE_B + PERIPHERALS_SIZE_B + L2_MEM_SIZE_B + CONFIG_SIZE_B)) {
    // Config: both Demux and RAB
    base_addr = CONFIG_BASE_ADDR;
    off = off - L3_MEM_SIZE_B - CLUSTERS_SIZE_B - PERIPHERALS_SIZE_B - L2_MEM_SIZE_B;
    size_b = CONFIG_SIZE_B;
    strcpy(type,"Config");
  }
  else if (off < (L3_MEM_SIZE_B + CLUSTERS_SIZE_B + PERIPHERALS_SIZE_B + L2_MEM_SIZE_B + CONFIG_SIZE_B + SLCR_SIZE_B)) {
    // SLCR
    base_addr = SLCR_BASE_ADDR;
    off = off - L3_MEM_SIZE_B - CLUSTERS_SIZE_B - PERIPHERALS_SIZE_B - L2_MEM_SIZE_B - CONFIG_SIZE_B;
    size_b = SLCR_SIZE_B;
