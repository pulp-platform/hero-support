...
int main(int argc, char *argv[]){
  ...
  printf("Profiling RAB miss handling...\n");      
  ...
  if (argc > 5) {
    printf("WARNING: More than 4 command line argument is not supported. Those will be ignored.\n");
  }
  ...
  for (i=0; i<N_CORES; i++) {
	...
	status = pulp_read32(pulp->mailbox.v_addr, MAILBOX_STATUS_OFFSET_B, 'b') & 0x2;	
      ...
      //printf("Wrote address %#x to mailbox.\n",(unsigned)array_ptr+PAGE_SIZE*(i*N_ELEMENTS_PER_CORE+j));
    }
  }
