...
#ifdef PULP
	int max_depth = crForest.vTrees[0]->GetDepth();
	int n_nodes_per_tree = (int)pow(2.0,int(max_depth+1))-1;
	int n_trees = crForest.GetSize();
	int n_leafs = 0, n_leafs_tot = 0;
	int n_patches = 0, n_patches_tot = 0;

	LeafNode* leaf;

	printf("\n# trees = %d\n",n_trees);

	//printf("Tree table size (per tree)  [Bytes] = %#x\n", n_nodes_per_tree*7*4);
	for (int i=0; i<n_trees; i++) {
	  n_leafs = crForest.vTrees[i]->num_leaf;
	  n_leafs_tot += n_leafs;
	  n_patches = 0;

	  leaf = crForest.vTrees[i]->leaf;
	  for (int j=0; j<n_leafs; j++) {
	    n_patches += leaf[j].vCenter.size(); 
	  }
	  n_patches_tot += n_patches;

	  printf("Tree %d has %d leafs with total %d patches.\n",i,n_leafs,n_patches);  
	}

	printf("Tree size (entire forest) [MiB] = %.3f\n",
	       float((n_nodes_per_tree*7*4+sizeof(CRTree))*n_trees)/(1024*1024));
	
	printf("Leaf size (entire forest) [MiB] = %.3f\n",
	       float(n_leafs_tot*sizeof(LeafNode)+n_patches_tot*sizeof(CvPoint))/(1024*1024));
	printf("\n");
#endif
