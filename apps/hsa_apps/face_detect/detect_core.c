...
void detect_core(const IMG_image_t * input, detect_param_t * param, core_param_t * core_param, RectList_t * pList)
{
  ...
  //DEBUG("[detect_core] %d, %d, %d , %d", core_param->detector->width, core_param->xinc, input->width, liiwWidth);
  ...
  //    DEBUG("[detect_core] %d stripe(s) to process / %d position per stripe", stripeNumber, integral_param.stripePositions);
  ...
  for(x=xStart ; x<xEnd ; x+=xStep) {
	...
	if (check_stat(currentCascade, pixSum, sqPixSum, stdDev) || core_param->disableStats) {
	  ...
	  if(north){
	    stop = execute_cascade_liiw(integral_param.integral_image, x, y, xStep, yStep, core_param->scanOrder, currentCascade, core_param->detector->width - 1, core_param->detector->height - 1, integral_param.currentWidth, yOff, Norm, core_param->ScalFactor, pList, c, NORTH);
	  }
	  if(!stop && west){
	    stop = execute_cascade_liiw(integral_param.integral_image, x, y, xStep, yStep, core_param->scanOrder, currentCascade, core_param->detector->width - 1, core_param->detector->height - 1, integral_param.currentWidth, yOff, Norm, core_param->ScalFactor, pList, c, WEST);
	  }
	  if(!stop && east){
	    stop = execute_cascade_liiw(integral_param.integral_image, x, y, xStep, yStep, core_param->scanOrder, currentCascade, core_param->detector->width - 1, core_param->detector->height - 1, integral_param.currentWidth, yOff, Norm, core_param->ScalFactor, pList, c, EAST);
	  }
	  if(!stop && south){
	    stop = execute_cascade_liiw(integral_param.integral_image, x, y, xStep, yStep, core_param->scanOrder, currentCascade, core_param->detector->width - 1, core_param->detector->height - 1, integral_param.currentWidth, yOff, Norm, core_param->ScalFactor, pList, c, SOUTH);
	  }
	} // if check_stat
      } // for NbCascades
    } // for yEnd
