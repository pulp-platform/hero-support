...
//              RectList_t * pList, int8_t cascade, int rotation)
{
  ...
  if (HitCount > 0) {
    ...
    printf("x+sx = %d, y+sy = %d, HitCount = %d\n", (int)(x+sx), (int)(y+sy), (int)HitCount);
...
void execute_cascade(const uint32_t * integral_image, uint32_t x, uint32_t y, cascade_t * currentCascade, int32_t wWindow, int32_t yOff, int32_t patchWidth, int32_t patchHeight, int32_t Norm, result_t *pResult, int rotation)
{
  ...
  while (pResult->level < currentCascade->nb_stage){
    ...
    do { //for weak
      ...
      //}
      //if ( yOff == 0 )
      //  printf("Sigma = %d, StrongThr = %d\n", Sigma, StrongThr);
      //if ( yOff == 0 )
      //  printf("AlphaSum = %d, StrongThr = %d, Sigma = %d\n", AlphaSum, StrongThr, Sigma);
      ...
      WeakNo += 1;
    } while (WeakNo < WeaksOffset + nWeak); // do

    //if ( yOff == 0)
    //  printf("WeakNo = %d, WeaksOffset = %d, nWeak = %d\n", WeakNo, WeaksOffset, nWeak);
