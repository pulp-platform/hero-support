...
if [ "${PLATFORM}" -eq "4" ]; then
    ...
    export KERNEL_DIR=${HERO_SDK}/linaro-${LINARO_RELEASE}/workspace/linux/out/juno-oe
    ...
    export ARM_LIB_EXT_DIR=${HERO_SDK}/libs-juno/lib
    export ARM_LIBC_DIR=${HERO_SDK}/oe-${OE_RELEASE}/openembedded/build/tmp-glibc/sysroots/lib32-genericarmv8/lib
    export ARM_LIB_FS_DIR=${HERO_SDK}/oe-${OE_RELEASE}/openembedded/build/tmp-glibc/sysroots/lib32-genericarmv8/usr
    export ARM_SYSROOT_DIR=${HERO_SDK}/oe-${OE_RELEASE}/openembedded/build/tmp-glibc/sysroots/lib32-genericarmv8
    ...
    export PATH=${HERO_SDK}/cross/linaro_gcc_${GCC_VERSION}/aarch64-linux-gnu/bin/:${HERO_SDK}/cross/linaro_gcc_${GCC_VERSION}/arm-linux-gnueabihf/bin/:$PATH
    ...
    export HERO_SDK=/scratch/vogelpi/te0808

    # directory containing the kernel sources
    #export KERNEL_DIR=${HERO_SDK}/petalinux/TEBF0808/os/wimmis02/build/linux/kernel/link-to-kernel-build
    ...
    # path to ARM libraries: external (opencv, ffmpeg), libc, shared libs in filesystem
    export ARM_LIB_EXT_DIR=${HERO_SDK}/../libs-zynqmp/lib
    #export ARM_LIBC_DIR=${HERO_SDK}/petalinux/TEBF0808/os/wimmis02/build/linux/rootfs/targetroot/lib
    #export ARM_LIB_FS_DIR=${HERO_SDK}/petalinux/TEBF0808/os/wimmis02/build/linux/rootfs/targetroot/usr
    ...
    export PULP_INC_DIR1=${HERO_SDK}/pulp-sdk/pkg/sdk/dev/install/include/archi/chips/bigpulp
    export PULP_INC_DIR2=${HERO_SDK}/pulp-sdk/pkg/sdk/dev/install/include
else 
    echo "Configuring for ZYNQ platform"

    # system workspace directory - on CentOS machine
    if   [ "${PLATFORM}" -eq "1" ]; then
        # export HERO_SDK=/scratch/vogelpi/zedboard
        export PULP_CURRENT_CONFIG="system=bigpulp:platform=hsa:nb_cluster=1"
    elif [ "${PLATFORM}" -eq "2" ]; then
        # export HERO_SDK=/scratch/vogelpi/zc706
        export PULP_CURRENT_CONFIG="system=bigpulp-z-7045:platform=hsa:nb_cluster=1"
    else
        # export HERO_SDK=/scratch/vogelpi/mini-itx
        export PULP_CURRENT_CONFIG="system=bigpulp-zux:platform=hsa:nb_cluster=1"
    fi

    # directory containing the kernel sources
    export KERNEL_DIR=${HERO_SDK}/workspace/linux-xlnx
