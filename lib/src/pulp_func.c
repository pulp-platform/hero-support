...
void pulp_rab_mh_enable(const PulpDev *pulp, unsigned char use_acp, unsigned char rab_mh_lvl)
{
...
int pulp_offload_pass_desc(PulpDev *pulp, const TaskDesc *task, const unsigned **data_idxs)
{
  ...
  if (i != n_data)
    printf("ERROR: Passed only %d of %d data elements to PULP.\n", i, n_data);
 ...
 * @n_idxs:    number of shared data elements passed by reference
 */
int pulp_offload_get_desc(const PulpDev *pulp, TaskDesc *task, const unsigned **data_idxs, int n_idxs)
{
  int i,j, n_data;
  unsigned *buffer;
